...
@onready var text_edit = $UI/Label/TextEdit
...
func _ready():
	...
	for column in range(column_count):
		...
		for row in range(row_count):
			...
			if(rng.randi_range(0,1) and !is_edge(column, row)):
				cell.visible = true
				previous_cell_state[column].push_back(true)
			else: previous_cell_state[column].push_back(false)
			cell_matrix[column].push_back(cell)
	text_edit.text = str(slow_factor)
	pass
...
func _on_text_edit_text_submitted(new_text):
	if(!str(new_text).is_valid_int()):
		text_edit.text = str(slow_factor)
	else:
		slow_factor = int(new_text)
		text_edit.text = str(slow_factor)
